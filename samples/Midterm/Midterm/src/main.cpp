...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		float     lightAmbientPow = 0.8f;
		float     lightSpecularPow = 4.0f;
		...
		BackendHandler::imGuiCallbacks.push_back([&]() {
			if (ImGui::CollapsingHeader("Effect controls"))
			{
				...
				if (activeEffect == 3)
				{
					...
					float thre = temp->GetThreshold();

					if (ImGui::SliderFloat("Threshold", &thre, 0.0f, 1.0f))
					{
						temp->SetThreshold(thre);
					}
					BloomEffect* temp2 = (BloomEffect*)effects[activeEffect];
					float pass = temp2->GetPasses();

					if (ImGui::SliderFloat("Passes", &pass, 0.0f, 10.0f))
					{
						temp2->SetPasses(pass);
					}
				}
			}
		...
		Texture2D::sptr Swor = Texture2D::LoadFromFile("images/Sword.png");
		...
		grassMat->Set("s_Environment", environmentMap);
		grassMat->Set("u_LightPos", lightPos);
		grassMat->Set("u_LightCol", lightCol);
		grassMat->Set("u_AmbientLightStrength", lightAmbientPow);
		grassMat->Set("u_SpecularLightStrength", lightSpecularPow);
		grassMat->Set("u_AmbientCol", ambientCol);
		grassMat->Set("u_AmbientStrength", ambientPow);
		grassMat->Set("u_LightAttenuationConstant", 1.0f);
		grassMat->Set("u_LightAttenuationLinear", lightLinearFalloff);
		grassMat->Set("u_LightAttenuationQuadratic", lightQuadraticFalloff);
		...
		simpleFloraMat->Set("s_Environment", environmentMap);
		simpleFloraMat->Set("u_LightPos", lightPos);
		simpleFloraMat->Set("u_LightCol", lightCol);
		simpleFloraMat->Set("u_AmbientLightStrength", lightAmbientPow);
		simpleFloraMat->Set("u_SpecularLightStrength", lightSpecularPow);
		simpleFloraMat->Set("u_AmbientCol", ambientCol);
		simpleFloraMat->Set("u_AmbientStrength", ambientPow);
		simpleFloraMat->Set("u_LightAttenuationConstant", 1.0f);
		simpleFloraMat->Set("u_LightAttenuationLinear", lightLinearFalloff);
		simpleFloraMat->Set("u_LightAttenuationQuadratic", lightQuadraticFalloff);
		simpleFloraMat->Set("u_Shininess", 8.0f);
		simpleFloraMat->Set("u_TextureMix", 0.0f);

		ShaderMaterial::sptr Sword = ShaderMaterial::Create();
		Sword->Shader = shader;
		Sword->Set("s_Diffuse", Swor);
		Sword->Set("s_Specular", noSpec);
		Sword->Set("s_Environment", environmentMap);
		Sword->Set("u_LightPos", lightPos);
		Sword->Set("u_LightCol", lightCol);
		Sword->Set("u_AmbientLightStrength", lightAmbientPow);
		Sword->Set("u_SpecularLightStrength", lightSpecularPow);
		Sword->Set("u_AmbientCol", ambientCol);
		Sword->Set("u_AmbientStrength", ambientPow);
		Sword->Set("u_LightAttenuationConstant", 1.0f);
		Sword->Set("u_LightAttenuationLinear", lightLinearFalloff);
		Sword->Set("u_LightAttenuationQuadratic", lightQuadraticFalloff);
		Sword->Set("u_Shininess", 8.0f);
		Sword->Set("u_TextureMix", 0.5f);
		...
		GameObject obj2 = scene->CreateEntity("sword");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/Sword.obj");
			obj2.emplace<RendererComponent>().SetMesh(vao).SetMaterial(Sword);
		...
		GameObject obj3 = scene->CreateEntity("sword");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/Stone.obj");
			obj3.emplace<RendererComponent>().SetMesh(vao).SetMaterial(Sword);
			obj3.get<Transform>().SetLocalPosition(0.0f, 0.0f, 0.0f);
			obj3.get<Transform>().SetLocalRotation(90.0f, 0.0f, 90.0f);
			obj3.get<Transform>().SetLocalScale(2.0f, 2.0f, 2.0f);
		}
		...
		EnvironmentGenerator::AddObjectToGeneration("models/simplePine.obj", simpleFloraMat, 250,
		...
		while (!glfwWindowShouldClose(BackendHandler::window)) {
			...
			obj2.get<Transform>().SetLocalPosition(0.0f,0.0f, pos);
